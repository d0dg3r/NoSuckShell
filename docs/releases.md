...
- **Target tag:** `v0.3.4` — push when you are ready; in-repo versions in `apps/desktop/package.json`, `apps/desktop/package-lock.json`, `apps/desktop/src-tauri/tauri.conf.json`, and `apps/desktop/src-tauri/Cargo.toml` must match **before** you tag (see [CHANGELOG.md](CHANGELOG.md)).
- **What ships:** see [CHANGELOG.md](CHANGELOG.md) for `0.3.4`.
