...
import { closeSession, deleteHost, deleteLayoutProfile, exportBackup, importBackup, listLayoutProfiles, listViewProfiles, listHostMetadata, listHosts, saveHost, saveHostMetadata, saveLayoutProfile, saveViewProfile, deleteViewProfile, reorderViewProfiles, sendInput, startSession, touchHostLastUsed, } from "./tauri-api";
...
export function App() {
            ...
            return (_jsxs("div", { "data-pane-index": paneIndex, className: `split-pane ${activePaneIndex === paneIndex ? "is-focused" : ""} ${dragOverPaneIndex === paneIndex ? "is-drag-over" : ""} ${paneSessionId ? "is-connected" : "is-empty"} ${isHoverTarget ? "is-host-hover-target" : ""} ${isHoverDimmed ? "is-host-hover-dimmed" : ""} ${hoveredHostAlias ? "is-host-hovering" : ""}`, draggable: false, onClick: () => {
                    ...
                    setContextMenu({
                        ...
                        paneIndex,
                                        ...
                                        }, children: _jsx("span", { "aria-hidden": "true", children: "\u25C9" }) }), _jsx("button", { className: `btn action-icon-btn pane-toolbar-btn ${allVisibleAlreadyTargeted ? "is-broadcast-active" : ""}`, title: "Target all visible panes", "aria-label": "Target all visible panes", disabled: !isBroadcastModeEnabled, onPointerDown: (event) => event.stopPropagation(), onClick: (event) => {
                                            event.stopPropagation();
    ...
    const renderHostRow = (row, key) => (_jsxs("div", { className: "host-row", children: [_jsx("button", { className: `host-favorite-btn host-favorite-btn-inline ${row.metadata.favorite ? "is-active" : ""}`, "aria-label": `Toggle favorite for ${row.host.host}`, onClick: (event) => {
                ...
                }, children: "\u2605" }), _jsxs("button", { className: `host-item ${row.connected ? "is-connected" : "is-disconnected"}`, onMouseEnter: () => setHoveredHostAlias(row.host.host), onMouseLeave: () => setHoveredHostAlias((prev) => (prev === row.host.host ? null : prev)), onDoubleClick: () => {
