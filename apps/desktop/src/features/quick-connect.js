...
export const parseHostPortInput = (value) => {
...
export const parseQuickConnectCommandInput = (value) => {
    const trimmed = value.trim();
    if (!trimmed) {
...
export const buildQuickConnectUserCandidates = (defaultUser, storedUsers) => {
    const result = [];
    const seen = new Set();
    const add = (value) => {
        const trimmed = value.trim();
