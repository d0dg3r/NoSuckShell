...
import { buildProxmoxNodeShellWebSocketUrl, parseProxmoxConsoleProxyData } from "../features/proxmox-console-ws";
...
type Props = {
  ...
  /** WebSocket open wait; defaults from app preferences constant when omitted (standalone window). */
  connectTimeoutMs?: number;
  onError?: (message: string) => void;
};

/** Same pve-xtermjs wire protocol as ProxmoxLxcTermPane. */
function attachProxmoxNodeShellSocket(
  term: Terminal,
  ws: WebSocket,
  apiUser: string,
  vncTicket: string,
...
export function ProxmoxNodeTermPane({
  clusterId,
  node,
  paneTitle,
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const raw = (await pluginInvoke(PROXMUX_PLUGIN_ID, "fetchNodeTermProxy", {
          clusterId,
          node,
        ...
        if (!raw?.ok || typeof raw.apiOrigin !== "string" || !raw.apiOrigin.trim()) {
          throw new Error("Invalid response from fetchNodeTermProxy.");
        }
        ...
        if (!ticket) {
          throw new Error("Node termproxy response missing port or ticket.");
        }
        const vncTicket = String(ticket.ticket);
        const wssUrl = buildProxmoxNodeShellWebSocketUrl(raw.apiOrigin.trim(), node, ticket);
        ...
        detachRef.current = attachProxmoxNodeShellSocket(term, ws, apiUser, vncTicket);
        fit.fit();
        setPhase("ready");
        setStatusMessage("");
      } catch (e) {
  ...
  }, [allowInsecureTls, clusterId, connectTimeoutMs, node, onError, reconnectRequestNonce, teardown, tlsTrustedCertPem]);

  return (
    <div className="proxmox-console-pane proxmox-console-pane--node-shell terminal-root terminal-host" role="region" aria-label={paneTitle}>
